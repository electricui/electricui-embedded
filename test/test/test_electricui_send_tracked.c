...
void test_send_tracked_small( void )
{
    encode_packet_simple_ExpectAnyArgsAndReturn(0);
    send_tracked(&stub_output_func, &short_payload, &default_settings );
}

void test_send_tracked_boundary_length( void )
{
    encode_packet_simple_ExpectAnyArgsAndReturn(0);
    send_tracked(&stub_output_func, &exact_payload, &default_settings );
}

void test_send_tracked_large( void )
{
    ...
    send_tracked(&stub_output_func, &large_payload, &default_settings );
}
